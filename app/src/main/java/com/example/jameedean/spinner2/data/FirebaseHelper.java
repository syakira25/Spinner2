package com.example.jameedean.spinner2.data;
...
public class FirebaseHelper {
    ...
    //SAVE
    public  Boolean save(AgencyModel spacecraft)
    {
        if(spacecraft==null)
        {
            saved=false;
        }else
        {
            try
            {
                db.child("agency").push().setValue(spacecraft);
                saved=true;
    ...
    private void fetchData(DataSnapshot snapshot,ArrayList<String> spacecrafts)
    {
        ...
        {
            String name=ds.getValue(AgencyModel.class).getName();
